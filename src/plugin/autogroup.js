...
const text = m.body.slice(prefix.length + cmd.length).trim();
    ...
    if (!m.isGroup) return m.reply("*This command can only be used in group idiot💀💀,,🖕*");
    ...
    if (!botAdmin) return m.reply("*Promote me to be admin*");
    if (!senderAdmin) return m.reply("*You must be admin to access this command idiot💀💀*");
    ...
    if (!time) {
      if (groupSetting === 'close') {
        ...
        return m.reply("Group closed successfully.");
      } else if (groupSetting === 'open') {
        await gss.groupSettingUpdate(m.from, 'not_announcement');
        return m.reply("Group opened successfully.");
      } else {
