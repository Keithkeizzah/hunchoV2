...
const text = m.body.slice(prefix.length + cmd.length).trim();
    ...
    if (!botAdmin) return m.reply("*Promote bot to be admin*");
    if (!senderAdmin) return m.reply("*You must ve admin to access bot commands*");
    ...
    if (users.length === 0) {
      return m.reply("*Mention a user to demote*");
    }
